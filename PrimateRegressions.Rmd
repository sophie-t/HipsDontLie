...
lmPelvic <- lm(Abduction ~ PelvicBreadth, data = pri.dat)
```
...
# Your turn:
summary(lmFemurHead)
#less than 0.05 (significant relationship)
summary(lmNSA)
#less than 0.05 (significant relationship)
summary(lmNeck)
#above 0.05
...
```{r}
#FemurHead, NSA (neck shaft angle), FTA, SUP, AcetabDiameter, PelvicBreadth
```
...
```{r}
# FTA (0.41)
#all the values were very low or negative
```
...
plot(pri.dat$AcetabDiameter, pri.dat$Abduction)
abline(lmAcetab)
...
```{r}
#NSA (positive), FTA (horizontal line), SUP (negative)
```
...
```{r}
#They are the same ROM variables. FemurHead, AcetabDiameter, PelvicBreadth were not seen as a strong correlation after looking at the graph.  
```
